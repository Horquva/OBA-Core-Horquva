...
router.get('/', async (req, res) => {
  try {
    ...
    res.json(packageIntelligence({
      ...
      capability:      'organizational_intelligence_fusion',
      findings,
      confidence:      (snap.trust_score ?? 0) / 100,
      evidence: [
        `Organizational Intelligence Score: ${snap.organizational_intelligence_score}/100`,
        `Rating: ${snap.rating}`,
        `Brain Posture: ${snap.brain_posture ?? 'unknown'}`,
        `Trust Score (signal coverage): ${snap.trust_score ?? 0}/100`
      ],
      recommendations: Array.isArray(snap.executive_recommendations)
