...
router.get('/summary', async (req, res) => {
  try {
    ...
    res.json(packageIntelligence({
      ...
      capability:      'learning_maturity_assessment',
      findings,
      confidence:      snapshot.learning_maturity_score / 100,
      evidence: [
        `Learning maturity level: ${formatLevel(snapshot.learning_maturity_level)} (score ${snapshot.learning_maturity_score})`,
        `${snapshot.mitigated_risks} of ${snapshot.total_known_risks} known risks mitigated (${snapshot.mitigation_percentage}%)`,
        `${repeatOffenders.length} repeat offender patterns detected`
      ],
      recommendations: [
        repeatOffenders.length > 0
          ? `Address ${repeatOffenders.length} repeat-offender assets to break failure cycles`
          : 'No repeat offenders — maintain current remediation cadence',
        highestExposureDept
          ? `Prioritize ${highestExposureDept.department} department — highest incident exposure score (${highestExposureDept.incident_exposure_score})`
          : 'All departments within acceptable exposure thresholds'
      ],
      graphRefs: highestExposureDept
        ? [{ nodeType: 'department', entityName: highestExposureDept.department }]
        : []
    }))
  } catch (err) {
