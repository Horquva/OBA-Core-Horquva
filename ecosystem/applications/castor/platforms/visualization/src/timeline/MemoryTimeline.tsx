...
export function MemoryTimeline({
  ...
  const sortedEvents = useMemo(
    () =>
      [...events].sort(
        (first, second) => getTimestamp(first) - getTimestamp(second),
      ),
    [events],
  );
  ...
  const filteredEvents = useMemo(() => {
    ...
    return sortedEvents.filter((event) => {
      ...
      const matchesStartDate =
        ...
        eventTimestamp >= selectedStartTimestamp;

      const matchesEndDate =
        selectedEndTimestamp === undefined ||
        eventTimestamp <= selectedEndTimestamp;

      const matchesScrubber =
        !timelineBounds || eventTimestamp <= scrubTimestamp;
