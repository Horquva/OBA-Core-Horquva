...
class KnowledgeServiceModel(Base):
    ...
    idempotency_payload_hash = Column(String, nullable=False)
...
class AntresKnowledgeServiceEngine:
    ...
    def ingest_knowledge(db: Session, request_payload: KnowledgeIngestRequest, idempotency_key: str) -> dict:
        ...
        # accidentally persisted as active operationalized knowledge.
        if not request_payload.validation.constitutional_check_passed or request_payload.validation.validation_status != "APPROVED":
            logger.error(f"Integrity Violation: Object '{request_payload.id}' failed constitutional or approval validation.")
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Integrity Safeguard Block: Unvalidated or unconstitutional knowledge cannot be operationalized."
            )

        # Fingerprint the payload so a reused idempotency key with different content
        # is detected instead of silently returning stale data.
        payload_hash = hashlib.sha256(
            json.dumps(request_payload.model_dump(), sort_keys=True, default=str).encode("utf-8")
        ).hexdigest()

        # 1. Idempotency Check
        existing_by_key = db.query(KnowledgeServiceModel).filter(KnowledgeServiceModel.idempotency_key == idempotency_key).first()
        if existing_by_key:
            if existing_by_key.idempotency_payload_hash != payload_hash:
                logger.error(f"Idempotency Key Reuse Conflict: key '{idempotency_key}' was previously used with a different payload.")
                raise HTTPException(
                    status_code=status.HTTP_409_CONFLICT,
                    detail=f"Idempotency key '{idempotency_key}' was already used with a different request payload. Use a new idempotency key for different content."
                )
