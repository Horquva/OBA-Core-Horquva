...
class LiveKnowledgePipeline:
    ...
    def process_and_persist(db: Session, raw: RawKnowledgeInput) -> OperationalKnowledgeModel:
        # Integrity Safeguard: raw knowledge must be governance-APPROVED and
        # constitutionally compliant before it can be operationalized. Without this
        # check, rejected/unconstitutional input would silently become "trusted"
        # operational knowledge.
        if not raw.validation.constitutional_check_passed or raw.validation.validation_status != "APPROVED":
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Integrity Safeguard Block: Unvalidated or unconstitutional knowledge cannot enter the live operationalization pipeline."
            )
