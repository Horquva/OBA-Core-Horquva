...
class KnowledgeGraphNodeModel(Base):
    ...
    constitutional_check_passed = Column(Boolean, nullable=False, default=True)
...
class KnowledgeGraphService:
    ...
    def create_node(db: Session, data: GraphNodeCreate) -> KnowledgeGraphNodeModel:
        ...
        # accidentally persisted as an active, retrievable graph node.
        if not data.constitutional_check_passed or data.validation_status != "APPROVED":
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Integrity Safeguard Block: Unvalidated or unconstitutional knowledge cannot be added to the graph."
            )
        ...
        node = KnowledgeGraphNodeModel(
            ...
            constitutional_check_passed=data.constitutional_check_passed,
