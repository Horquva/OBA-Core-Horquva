...
def aggregate_evidence(
    ...
    _arc_root = (
        ...
        if arcturus_root
        else _this_file.parents[4]  # src/integration/../../../ = arcturus root
    )
    _repo_root = (
        Path(repo_root).resolve()
        if repo_root
        else _this_file.parents[6]  # repo root
    )
...
def _collect_platform_evidence(
    ...
    for rel_path in all_expected:
        full = arc_root / rel_path
        category = rel_path.split("/")[0]
        if full.exists():
            if category in ("contracts", "schemas"):
                ev.contracts_present.append(rel_path)
            elif category == "src":
                ev.src_files_present.append(rel_path)
            elif category == "tests":
                ev.test_files_present.append(rel_path)
        else:
            if category in ("contracts", "schemas"):
                ev.contracts_missing.append(rel_path)
            elif category == "src":
                ev.src_files_missing.append(rel_path)
            elif category == "tests":
                ev.test_files_missing.append(rel_path)
...
def _run_pytest(arc_root: Path) -> tuple[int | None, str]:
    ...
    try:
        result = subprocess.run(
            ...
            cwd=arc_root.parents[3],  # repo root
            capture_output=True,
            text=True,
            timeout=120,
        )
