...
Covers: happy-path execution, deterministic results, and typed-error
rejection for invalid inputs/state transitions.
"""
...
def test_full_lifecycle_produces_completed_record():
    engine = make_engine()
    ctx = SimulationContext(experiment_id="EXP-TEST-001", seed=7)
...
def test_same_seed_produces_same_config_snapshot():
    ctx_a = SimulationContext(experiment_id="EXP-TEST-002", seed=99, config={"x": 1})
    ctx_b = SimulationContext(experiment_id="EXP-TEST-002", seed=99, config={"x": 1})
...
# ---------------------------------------------------------------------------
# Typed-error rejection (Day 4 requirement: fail safely, never crash silently)
# ---------------------------------------------------------------------------
...
def test_double_initialize_raises_business_rule_violation():
    engine = make_engine()
    ctx = SimulationContext(experiment_id="EXP-TEST-003", seed=1)
    engine.initialize_run(ctx)
    with pytest.raises(BusinessRuleViolation):
        engine.initialize_run(ctx)
...
def test_invalid_seed_rejected_by_schema():
    with pytest.raises(ValidationError):
        SimulationContext(experiment_id="EXP-TEST-004", seed=-5)
