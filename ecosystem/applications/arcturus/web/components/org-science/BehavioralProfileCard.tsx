...
import { orgScience, ApiError, type IntelligenceResponse, type BehaviorPayload } from '../../lib/api';
...
export function BehavioralProfileCard() {
  const [res, setRes] = useState<IntelligenceResponse<BehaviorPayload> | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const response = await orgScience.behavior();
        if (cancelled) return;
        setRes(response);
        setState('success');
      } catch (err: unknown) {
    ...
    <div className="card border-[var(--border-subtle)] flex flex-col h-full min-h-[280px]">
      <div className="px-6 py-4 border-b border-[var(--border-subtle)] flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <h3 className="text-sm font-semibold text-[color:var(--text-primary)]">Behavioral Profile</h3>
        </div>
        {state === 'success' && data && (
      ...
      <div className="p-6 flex-1 flex flex-col justify-center">
        ...
        {state === 'error' && (
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm text-[color:var(--text-primary)] font-medium mb-1">Failed to load behavior profile</p>
        ...
        {state === 'empty' && (
          <div className="flex flex-col items-center text-center">
            <User className="w-8 h-8 text-[color:var(--text-tertiary)] mb-2" />
            <p className="text-sm text-[color:var(--text-secondary)]">No behavior data available.</p>
          </div>
        )}
