...
import { orgScience, ApiError, type IntelligenceResponse, type PatternPayload } from '../../lib/api';
...
export function PatternRegularityCard() {
  const [res, setRes] = useState<IntelligenceResponse<PatternPayload> | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const response = await orgScience.pattern();
        if (cancelled) return;
        setRes(response);
        setState('success');
      } catch (err: unknown) {
        ...
        setErrorMsg(
          err instanceof ApiError
            ...
            : 'Failed to fetch pattern regularity',
        );
        setState('error');
      }
    }
    ...
    <div className="card border-[var(--border-subtle)] flex flex-col h-full min-h-[280px]">
      <div className="px-6 py-4 border-b border-[var(--border-subtle)] flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <h3 className="text-sm font-semibold text-[color:var(--text-primary)]">Pattern Regularity</h3>
        </div>
        {state === 'success' && data && (
          <span className={clsx(
            "px-2.5 py-1 rounded text-[10px] font-bold uppercase tracking-widest border",
            data.anomalyCount === 0 ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 
            data.anomalyCount > 3 ? 'bg-red-500/10 text-red-400 border-red-500/20' : 
            'bg-yellow-500/10 text-yellow-400 border-yellow-500/20'
          )}>
            {data.anomalyCount === 0 ? 'STABLE' : data.anomalyCount > 3 ? 'CRITICAL' : 'WARNING'}
          </span>
        )}
      </div>
      ...
      <div className="p-6 flex-1 flex flex-col justify-center">
        ...
        {state === 'error' && (
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm text-[color:var(--text-primary)] font-medium mb-1">Failed to load pattern data</p>
        ...
        {state === 'empty' && (
          <div className="flex flex-col items-center text-center">
            <Activity className="w-8 h-8 text-[color:var(--text-tertiary)] mb-2" />
            <p className="text-sm text-[color:var(--text-secondary)]">No pattern data available.</p>
          </div>
        )}
          ...
          <div className="space-y-5">
            ...
            <div>
              <MetricRow 
                ...
                value={data.dominantPattern.type} 
                color="text-[color:var(--text-primary)] uppercase text-xs" 
              />
              <MetricRow 
                label="Pattern Occurrences" 
                value={data.dominantPattern.count} 
