...
import { ShieldAlert, AlertTriangle, Users, Zap, CheckCircle2, XCircle, ChevronDown, ChevronUp, LayoutGrid } from 'lucide-react';
...
function CriticalAgentCard({ profile, rank }: CriticalAgentCardProps) {
  ...
  return (
    <div className={clsx(
      'card overflow-hidden border-[var(--border-subtle)] relative',
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between px-6 py-5 relative z-10 gap-5">
        <div className="flex items-start gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h3 className="text-base font-semibold text-[color:var(--text-primary)]">{agent.name}</h3>
            ...
            <div className="flex items-center gap-3 mt-1.5 text-xs text-[color:var(--text-secondary)] flex-wrap">
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-[var(--bg-hover)] border border-[var(--border-default)] text-[color:var(--text-primary)]">
                <LayoutGrid className="w-3 h-3 text-[color:var(--text-tertiary)]" />
                {agent.department}
              </span>
              {agent.owner ? (
              ...
              {agent.backup_owner ? (
                <span className="flex items-center gap-1">
                  ...
                  <span className="text-[color:var(--text-tertiary)]">Backup: {agent.backup_owner}</span>
                </span>
              ) : (
        ...
        <div className="flex items-center justify-between w-full md:w-auto md:flex-shrink-0 pt-4 md:pt-0 border-t md:border-t-0 border-[var(--border-subtle)] gap-4">
          {/* Score pill */}
          <div className="flex flex-col items-end md:items-end">
            <span className="text-[10px] text-[color:var(--text-tertiary)] uppercase tracking-widest mb-0.5">Risk Score</span>
            <span className="text-2xl font-bold text-red-400 leading-none">{compositeScore}</span>
          </div>
          {/* Expand toggle */}
          <button
            onClick={() => setExpanded(v => !v)}
            className="w-8 h-8 rounded-lg bg-[var(--bg-hover)] border border-[var(--border-default)] flex items-center justify-center text-[color:var(--text-secondary)] hover:text-[color:var(--text-primary)] hover:border-[var(--border-strong)] transition-all"
      ...
      {expanded && (
        <div className="border-t border-[var(--border-subtle)] px-6 py-5 relative z-10 animate-fade-in">
          ...
          <div className="space-y-2.5">
            {factors.map((factor, i) => (
              <div key={i} className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="text-sm text-[color:var(--text-primary)]">{factor.label}</span>
                </div>
                <div className={clsx(
                  ...
                  'text-[color:var(--text-secondary)] bg-[var(--bg-hover)] border-[var(--border-default)]'
                )}>
            ...
            <div className="border-t border-[var(--border-subtle)] pt-2.5 flex items-center justify-between">
              <span className="text-sm font-semibold text-[color:var(--text-primary)]">Total Risk Score</span>
              <span className="text-sm font-bold text-red-400">{compositeScore}</span>
            </div>
          </div>

          {downstreamCount > 0 && (
            <div className="mt-4 px-4 py-3 rounded-lg bg-[var(--bg-surface)] border border-[var(--border-subtle)] flex items-center gap-2.5">
  ...
  if (criticalAgents.length === 0) {
    return (
      <div className="card px-6 py-10 flex flex-col items-center justify-center text-center animate-fade-up delay-300">
        ...
        <h3 className="text-[color:var(--text-primary)] font-semibold mb-1">No Critical Agents</h3>
        <p className="text-sm text-[color:var(--text-secondary)]">All agents are below the CRITICAL risk threshold.</p>
      </div>
    );
  }
  ...
  return (
    <div className="animate-fade-up delay-300">
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          <h2 className="text-lg font-semibold text-[color:var(--text-primary)]">Critical Risk Agents</h2>
          <p className="text-xs text-[color:var(--text-secondary)]">
