...
// The actual bug this fixture used to hide: the allowed set was built from
// roots.platforms, which is always undefined (the real table is
// ai_platforms) — so a real platform name was flagged on every turn.
{
	const result = validateEntityAllowlist('Snowflake went down for maintenance last night.', allowed)
	check('does not flag a real platform name (roots.ai_platforms, not roots.platforms)', result.status === 'clean')
}

// knowledge_assets rows carry a `topic`, not a `name` — the allowed set
// must read the field that actually exists on the row.
check('builds allowed set from knowledge_assets\' topic field', allowed.has('Snowflake access policy'))

// There is no `departments` root table — department names come from the
// distinct values actually seen on employees/workflows.
check('builds allowed set from real department values (no departments table exists)', allowed.has('Engineering') && allowed.has('Data'))
