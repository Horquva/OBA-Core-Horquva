...
console.log('run_simulation:')
{
	...
	// The tool's own JSON schema declares targetId as a STRING, and a real
	// model call is bound by that schema — it does not know the underlying
	// roots id is a JS number. Without coercion, "1" !== 1 under
	// domain/simulations.js's strict-equality lookups, and every live
	// simulation call silently resolved to "no entity found". This is the
	// one existing tests never caught: they all called .run() with a
	// numeric literal that happened to match the fixture's own type.
	const stringId = runSimulationTool.run(ctx, { scenario: 'employee_leaves', targetId: '1' })
	check('a string targetId (what the declared schema actually produces) still resolves', stringId.data !== null, stringId)
...
console.log('\ncompare_scenarios:')
{
	...
	const stringCmp = compareScenariosTool.run(ctx, {
		scenarioA: { scenario: 'employee_leaves', targetId: '1' },
		scenarioB: { scenario: 'employee_leaves', targetId: '2' },
	})
	check('compare_scenarios also resolves string target ids from a live model call', stringCmp.data !== null, stringCmp)
}
