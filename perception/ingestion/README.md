...
Normalize the raw GitHub payload into the event contract shape. Assign `signal_id` here — this is the only place it is generated.

## Done when

A real event is normalized and carries a `signal_id`.
...
Do not classify. Classification is `evidence/classification/` (Maaz).
