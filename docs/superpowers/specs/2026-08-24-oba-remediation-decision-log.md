...
| `frontend/lib/graph.ts`'s `getSPOFs()` — a 4th SPOF implementation, still live | found 2026-08-26 while closing out backend SPOF unification (row above): requires >=3 downstream victims + no `backup_owner` + high/critical risk, computed client-side from raw fetched data. Still imported by `frontend/lib/riskIntelligence.ts` (drives the `/risk` page) and `frontend/components/simulation/ScenarioSandbox.tsx` (lower-stakes — only used there to pick a target to simulate, not to assert a SPOF verdict to the user). Same shape as W-I's frontend-repointing work, not done in the same pass as the backend fixes above | when frontend truth-repointing (prerequisite 3 of 4 for the AI-agent interface) is picked up, or sooner if `/risk` page accuracy matters before then |
