...
| ~~SPOF definition unification~~ — **resolved 2026-08-26, no owner decision was actually needed.** `definitions.js`'s `spofVerdict()` was already the canonical definition per D-06 (decided 2026-08-24) — the "three disagreeing definitions" finding from W-I's design phase was itself imprecise: `analytics.js`'s `singlePointsOfFailure()` is a graph-structural signal for the brain's M-modules, legitimately different from D-06's root-table entity SPOF per the same split D-12 already drew between graph-structural and root-aggregate questions, not a conflict to resolve. The one genuine gap — `routes/risks.js` computing its own ad hoc rule instead of calling `spofVerdict()` — is fixed (commit `4c08eab`). `routes/workflows/spof.js` remains separately blocked, see above, for a different reason (no backup concept in its schema, not a definitional disagreement) | closed |
