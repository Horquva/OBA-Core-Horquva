  ...
  return (
    <div className="animate-fade-up delay-300">
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          <h2 className="text-lg font-semibold text-white">Health Trajectory</h2>
          <p className="text-xs text-slate-400">Directional health score indicator over time</p>
        </div>
      </div>
      ...
      {state === 'loading' && (
        <div className="card px-6 py-6 border border-[#1f1f29]">
          <div className="flex items-center gap-6">
            {[0, 1, 2].map(i => (
              <div key={i} className="flex-1 h-12 rounded bg-[#1f1f29] animate-pulse-soft" />
            ))}
          </div>
        </div>
      )}
      ...
      {state === 'error' && (
        <div className="card px-6 py-6 border border-[#1f1f29]">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm text-white font-medium mb-1">Failed to load health trends</p>
      ...
      {state === 'empty' && (
        <div className="card px-6 py-10 flex flex-col items-center justify-center text-center">
          ...
          <h3 className="text-white font-semibold mb-1">No Trend Data</h3>
          <p className="text-sm text-slate-400">Forecast health trend data is currently unavailable.</p>
        </div>
      )}
