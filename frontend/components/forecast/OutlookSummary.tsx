  ...
  return (
    <div className="animate-fade-up delay-150">
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          <h2 className="text-lg font-semibold text-white">Organizational Outlook</h2>
          <p className="text-xs text-slate-400">30, 60, and 90-day trajectory forecasts</p>
        </div>
      </div>
        ...
        <div className="card overflow-hidden border border-[#1f1f29]">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[#111116] border-b border-[#1f1f29]">
                  {[0, 1, 2, 3, 4].map(i => (
                    <th key={i} className="px-5 py-3.5"><div className="h-4 w-20 rounded bg-[#1f1f29] animate-pulse-soft" /></th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-[#111116]">
                {[0, 1, 2].map(i => (
                  <tr key={i}>
                    {[0, 1, 2, 3, 4].map(j => (
                      <td key={j} className="px-5 py-4"><div className="h-4 w-24 rounded bg-[#1f1f29] animate-pulse-soft" /></td>
                    ))}
                  </tr>
                ))}
              </tbody>
      ...
      {state === 'error' && (
        <div className="card px-6 py-6 border border-[#1f1f29]">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm text-white font-medium mb-1">Failed to load outlook</p>
      ...
      {state === 'empty' && (
        <div className="card px-6 py-10 flex flex-col items-center justify-center text-center">
          ...
          <h3 className="text-white font-semibold mb-1">No Outlook Data</h3>
          <p className="text-sm text-slate-400">Forecast trajectories are not available.</p>
        </div>
      )}
      ...
      {state === 'success' && data && (
        <div className="space-y-6">
          ...
          <div className="card px-6 py-5 border border-[#1f1f29] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
            ...
            {data.summary.headlineOutlook.weakestDimension && (
              <div className="px-4 py-3 rounded-lg bg-[#111116] border border-[#28283a] flex items-center gap-3">
                <AlertTriangle className="w-4 h-4 text-amber-400" />
                <div>
                  <p className="text-[10px] text-slate-500 uppercase tracking-widest font-medium">Weakest Dimension</p>
                  <p className="text-sm text-slate-300 capitalize">{data.summary.headlineOutlook.weakestDimension}</p>
                </div>
              </div>
            )}
          </div>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-[#111116]">
                  {data.outlook.organizationalOutlook.map(row => (
                    <tr key={row.horizonDays} className="hover:bg-[#1a1a22] transition-colors">
                      <td className="px-5 py-4">
                        <span className="text-sm font-semibold text-white">{row.horizonDays} Days</span>
                      </td>
                      <td className="px-5 py-4">
