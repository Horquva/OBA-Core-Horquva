...
function StatusBar({ preserved, vulnerable, atRisk, lost, total }: {
  ...
  return (
    <div style={{ display: 'flex', height: '5px', borderRadius: '9999px', overflow: 'hidden', gap: '2px' }}>
      {pctP > 0 && <div style={{ width: `${pctP}%`, background: 'var(--risk-low-text)',      borderRadius: '9999px' }} />}
      {pctV > 0 && <div style={{ width: `${pctV}%`, background: 'var(--risk-high-text)',     borderRadius: '9999px' }} />}
      {pctA > 0 && <div style={{ width: `${pctA}%`, background: 'var(--risk-medium-text)',   borderRadius: '9999px' }} />}
      {pctL > 0 && <div style={{ width: `${pctL}%`, background: 'var(--risk-critical-text)', borderRadius: '9999px' }} />}
    </div>
  );
}
      ...
      {open && (
        <div style={{ borderTop: '1px solid var(--border-subtle)' }}>
          {carrier.assets.map((asset, i) => {
            ...
            return (
              <div
                key={asset.id || `asset-${asset.name}-${i}`}
                style={{
                  display: 'grid',
