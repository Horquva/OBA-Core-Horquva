...
import { CommandTarget, STATIC_TARGETS, searchTargets, defaultSuggestions } from "@/lib/commandIndex";
...
function entityToTarget(e: SearchEntry): CommandTarget {
  return {
    ...
    label: e.title,
    kind: "entity",
    page: e.type === "Agent" ? "/dashboard" : ENTITY_PAGE[e.type],
...
export default function GlobalSearchOverlay() {
  ...
  const pool = useMemo(() => [...STATIC_TARGETS, ...index.map(entityToTarget)], [index]);
  ...
  return (
    <div className="fixed inset-0 z-10 flex items-start justify-center pt-20 sm:pt-24">
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm" onClick={closeAllPanels} />
          ...
          {flat.length === 0 && (
            <div className="py-12 text-center">
              ...
              <p className="mt-2 text-xs" style={{ color: "var(--text-tertiary)" }}>
                Try a module code (M09), a block name (Knowledge Concentration Gauge), or a person, agent,
                workflow or tool.
              </p>
            </div>
          )}
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="block truncate text-xs" style={{ color: "var(--text-tertiary)" }}>
                        {t.kind === "page" ? t.hint : [t.parent, t.hint].filter(Boolean).join(" · ")}
                      </span>
                    </span>
