...
export function IndustryBenchmarkCard() {
  const [res, setRes] = useState<IntelligenceResponse<BenchmarkPayload> | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const response = await orgScience.benchmark();
        if (cancelled) return;
        setRes(response);
        setState('success');
      } catch (err: unknown) {
      ...
      <div className="p-6 flex-1 flex flex-col justify-center">
        ...
        {state === 'error' && (
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm text-[color:var(--text-primary)] font-medium mb-1">Failed to load benchmark</p>
        ...
        {state === 'empty' && (
          <div className="flex flex-col items-center text-center">
            <BarChart2 className="w-8 h-8 text-[color:var(--text-tertiary)] mb-2" />
            <p className="text-sm text-[color:var(--text-secondary)]">No benchmark data available.</p>
          </div>
        )}
          ...
          <div className="space-y-5">
            <div className="text-center">
              <div className="text-4xl font-bold text-[color:var(--text-primary)] tabular-nums tracking-tight mb-1">
                {Math.round(data.benchmarkScore * 100)}%
              </div>
