...
function MetricRow({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div className="flex items-center justify-between text-sm py-1.5 border-b border-[#1f1f29] last:border-0">
    ...
    <div className="card border-[#1f1f29] flex flex-col h-full min-h-[280px]">
      <div className="px-6 py-4 border-b border-[#1f1f29] flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <h3 className="text-sm font-semibold text-white">Collaboration Matrix</h3>
        </div>
        {state === 'success' && data && (
          <span className={clsx(
      ...
      <div className="p-6 flex-1 flex flex-col justify-center">
        {state === 'loading' && (
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <div className="h-6 w-full rounded bg-[#1f1f29] animate-pulse-soft" />
              <div className="h-6 w-full rounded bg-[#1f1f29] animate-pulse-soft" />
            </div>
          </div>
        )}
        ...
        {state === 'error' && (
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm text-white font-medium mb-1">Failed to load collaboration data</p>
        ...
        {state === 'empty' && (
          <div className="flex flex-col items-center text-center">
            ...
            <p className="text-sm text-slate-400">No collaboration data available.</p>
          </div>
        )}
