...
const STATUS_STYLES: Record<string, { color: string; bg: string; border: string }> = {
  ...
  PENDING:   { color: 'text-[color:var(--text-secondary)]',   bg: 'bg-slate-500/10',   border: 'border-slate-500/20' },
};
...
function SkeletonRow({ index }: { index: number }) {
  return (
    <tr key={index}>
      <td className="px-5 py-4"><div className="h-4 w-24 rounded bg-[var(--border-subtle)] animate-pulse-soft" /></td>
      <td className="px-5 py-4"><div className="h-4 w-32 rounded bg-[var(--border-subtle)] animate-pulse-soft" /></td>
      <td className="px-5 py-4"><div className="h-4 w-28 rounded bg-[var(--border-subtle)] animate-pulse-soft" /></td>
      <td className="px-5 py-4"><div className="h-4 w-20 rounded bg-[var(--border-subtle)] animate-pulse-soft" /></td>
      <td className="px-5 py-4"><div className="h-4 w-12 rounded bg-[var(--border-subtle)] animate-pulse-soft" /></td>
      <td className="px-5 py-4"><div className="h-4 w-20 rounded bg-[var(--border-subtle)] animate-pulse-soft ml-auto" /></td>
    </tr>
  );
}
    ...
    <div className="animate-fade-up delay-400">
      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-[color:var(--text-primary)]">Verification Ledger</h2>
            <p className="text-xs text-[color:var(--text-secondary)]">Action verification log with compliance status</p>
          </div>
        </div>

        {state === 'success' && (
          <div className="flex items-center gap-1 bg-[var(--border-subtle)] border border-[var(--border-default)] rounded-lg p-0.5">
            <button
              onClick={() => setFilter('all')}
              className={clsx(
                'px-3 py-1 rounded-md text-xs font-medium transition-all',
                filter === 'all'
                  ? 'bg-[var(--border-default)] text-[color:var(--text-primary)]'
      ...
      {state === 'empty' && (
        <div className="card px-6 py-10 flex flex-col items-center justify-center text-center">
          <FileCheck className="w-10 h-10 text-[color:var(--text-tertiary)] mb-3" />
          <h3 className="text-[color:var(--text-primary)] font-semibold mb-1">No Verification Actions</h3>
          <p className="text-sm text-[color:var(--text-secondary)]">No verification actions have been recorded yet.</p>
        </div>
      )}
      ...
      {state === 'error' && (
        <div className="card px-6 py-6 border border-[var(--border-subtle)]">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm text-[color:var(--text-primary)] font-medium mb-1">Failed to load verification data</p>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-[#111116]">
                {displayedActions.map((action, i) => {
                  ...
                  return (
                    <tr key={i} className="hover:bg-[var(--bg-hover)] transition-colors align-top">
                      {/* Actor */}
                      <td className="px-5 py-4">
                        <span className="text-sm text-[color:var(--text-primary)] font-medium">{action.actorName}</span>
                        <p className="text-[10px] text-[color:var(--text-tertiary)]">{action.actorType}</p>
                      </td>

                      {/* Action name */}
                      <td className="px-5 py-4">
                        <span className="text-sm text-[color:var(--text-primary)]">{action.actionName}</span>
                      </td>
      ...
      {state === 'success' && displayedActions.length === 0 && (
        <div className="card px-6 py-10 flex flex-col items-center justify-center text-center">
          <Filter className="w-10 h-10 text-[color:var(--text-tertiary)] mb-3" />
          <h3 className="text-[color:var(--text-primary)] font-semibold mb-1">No Flagged Actions</h3>
          <p className="text-sm text-[color:var(--text-secondary)]">No flagged actions found in the verification log.</p>
        </div>
      )}
    </div>
  );
